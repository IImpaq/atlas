...
PackageInstaller::PackageInstaller(const fs::path &cache, const fs::path &install, const fs::path &log,
  const PackageConfig &packageConfig): cacheDir(cache), installDir(install), logDir(log) {
#ifdef __APPLE__
  platform = "macos";
#else
        platform = "linux";
...
bool PackageInstaller::executeCommands(const Json::Value &commands) {
...
std::string PackageInstaller::replaceVariables(const std::string &cmd) {
...
bool PackageInstaller::downloadFile(const std::string &url, const std::string &target) {
